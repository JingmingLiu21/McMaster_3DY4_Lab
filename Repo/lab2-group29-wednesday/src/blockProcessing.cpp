...
void impulseResponseLPF(float Fs, float Fc, unsigned short int num_taps, std::vector<float> &h)
{
    ...
    for (int i=0; i<num_taps; i++) {
        if (i == halfway) {
            ...
            h[i] = norm_cutoff*((sin((double)PI*norm_cutoff*(i-halfway))) / (PI*norm_cutoff*(i-halfway)));
        }
...
void convolveFIR(std::vector<float> &y, const std::vector<float> &x, const std::vector<float> &h)
{
	...
	for (int i = 0; i < x.size();i++){
		...
		counter = 0;
		float sum = 0.0;
		for(int n=0; n< element[i].size(); n++){
		   sum += element[i][n];
		}
		y[i] = sum;
...
void block_filtering(const std::vector<float> &audio_data, std::vector<float> &audio_left, std::vector<float> &audio_right, float &Fs, float &Fc, unsigned short int &num_taps, int &block_size,  std::vector<float> &single_pass_left, std::vector<float> &single_pass_right) {
    std::vector<float> low_p_coe;
	std::vector<float> filtering_left;
	std::vector<float> filtering_right;
	std::vector<float> filtered_left;
	std::vector<float> filtered_right;



    filtered_left.resize(block_size);
    filtered_right.resize(block_size);
    filtering_left.resize(block_size);
    filtering_right.resize(block_size);
    single_pass_left.resize(audio_left.size());
    single_pass_right.resize(audio_right.size());
...
int main()
{
    ...
    block_filtering(audio_data, audio_left, audio_right, Fs, Fc, num_taps, block_size, single_pass_left, single_pass_right );
    std::vector<float> h;
    ...
    //printf("%d\n", single_pass_left);
    //printRealVector(single_pass_left);
